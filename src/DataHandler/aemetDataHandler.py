...
def get_aemet_climate_data(start_date: datetime, end_date: datetime, national_id: str) -> pd.DataFrame:
    """
    ...
    :return:
    """
    station_id_str = '/estacion/' + national_id
    request_json = 'server_answer.json'
    dates = monthly_date_range(start_date, end_date)
    data = pd.DataFrame()
    for month in dates:
        outfile = AEMET_STORAGE+'AEMET_CLIM_DAILY_'+national_id + '_' + month.strftime('%Y%m')
        if os.path.isfile(outfile):
            ...
            url_request = AEMET_BASE_URL + month.strftime('%Y-%m-%dT00:00:00UTC') + AEMET_MID_URL_2 + (month+timedelta(days=32)).replace(day=1).strftime('%Y-%m-%dT00:00:00UTC') + AEMET_MID_URL + station_id_str + AEMET_API_KEY
...
def get_aemet_reference_data():
    return pd.read_csv(AEMET_STORAGE+'reference_data/valoresclimatologicos_palma-de-mallorca-aeropuerto.csv',
                       header=1)