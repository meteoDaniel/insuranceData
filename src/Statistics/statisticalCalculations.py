...
def monthly_to_daily_mean_sunduration(reference_data: pd.DataFrame) -> pd.DataFrame:
    ...
    daily_sunshine_mean_reference = reference_data.I[:-1].values /\
                                    days_per_month
    ref_data_sun = pd.DataFrame(daily_sunshine_mean_reference,
                                index=monthly_date_range(datetime(2018, 1, 1),
                                                         datetime(2018, 12, 1)))

    ref_data_sun.index = ref_data_sun.index + pd.DateOffset(days=15)
    interpolated_reference_data = pd.DataFrame(index=daily_date_range(
        datetime(2018, 1, 1),
        datetime(2018, 12, 31)))
...
def calculcate_mean_days_lower_than_table(data: pd.DataFrame,
                                          year: int) -> pd.DataFrame:
    """
        calculate the mean number of days lower than 1-6 hours
        for all possible trip_duration between 7 and 21 days and all 365
        trip starting days
    """
...
def calculcate_num_days_lower_than_table(data: pd.DataFrame) -> pd.DataFrame:
    """
        calculate the number of days lower than 1-6 hours
        for all possible trip_duration between 7 and 21 days and all 365
        trip starting days
    """
    num_days_lower_day_per_year = create_reference_table(2018)
...
def poisson_probability(u: float, n: float) -> float:
    """
        formula for the poisson_probability
    """
